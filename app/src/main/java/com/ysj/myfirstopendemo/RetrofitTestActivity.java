...
public class RetrofitTestActivity extends AppCompatActivity {
    ...
    @OnClick({R.id.btn_get,R.id.btn_get_restful,R.id.btn_post})
    public void onViewClick(View view) {
        switch (view.getId()) {
            case R.id.btn_get:
                getByRetrogit();
                break;
            case R.id.btn_get_restful:
                getByRetrofitForRestful();
                break;
            case R.id.btn_post:
                postByRetrofit();
                break;
    ...
    private void postByRetrofit() {
        //1.使用建造者模式创建retrofit实例
        Retrofit retrofit = new Retrofit.Builder()
                .baseUrl(LOCALURL)
                .build();
        //2.使用retrofit对象调用create()创建服务实例
    ...
    private void getByRetrofitForRestful() {
        //1.使用建造者模式创建retrofit实例
        Retrofit retrofit = new Retrofit.Builder()
                .baseUrl(LOCALURL)
                .build();
        ...
        call.enqueue(new Callback<ResponseBody>() {
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                try {
                    String output = response.body().string();
                    Log.e(TAG, "GET请求发送成功！获取到的信息为：" + output);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
    ...
    private void getByRetrogit() {
        ...
        TestService testService = retrofit.create(TestService.class);
        // 3.通过服务实例创建Call对象
        Call<ResponseBody> call = testService.getInfoByHttp("15558167320");
        // 4.通过Call对象构建网络请求（异步）
        call.enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
